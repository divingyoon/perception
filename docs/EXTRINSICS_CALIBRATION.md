...
# 2) 마커가 보이는 프레임 + camera_info 저장 (컨테이너 안)
docker exec -it <컨테이너> bash -lc \
  'source /opt/ros/humble/setup.bash; \
   python3 /workspaces/isaac_ros-dev/isaac_ros_assets/tools/grab_frame.py \
     --out /tmp/calib.png --camera-info-out /tmp/camera_info.yaml'
docker cp <컨테이너>:/tmp/calib.png .
docker cp <컨테이너>:/tmp/camera_info.yaml .

# 3) extrinsics 계산 + yaml 기록 (host, opencv 있는 곳)
python3 tools/calibrate_extrinsics.py \
    --image calib.png --camera-info camera_info.yaml \
